  ...
  for (int i = 0; i < 16; i++) {
    ...
    if (test0 && test1 && test2 && test3) {
      ...
      asm volatile("cp.async.cg.shared.global [%0], [%1], %2;\n" ::"r"(
                       (uint32_t)__cvta_generic_to_shared(&C[tileIdx])),
...
__device__ void loadtileC_tune(MMAarguments &arg, ElementOutput *C_fragemnt) {
  ...
  for (int i = 0; i < 16; i++) {
    ...
    for (int j = 0; j < 2; j++) {
      int rowC =
          blockIdx.y * 128 + rowwarp * 64 + rowtile * 16 + laneidx / 4 + j * 8;
      int colC = blockIdx.x * 64 + colwarp * 32 + coltile * 8 + laneidx % 4 * 2;

      bool test0 = test(rowC, colC, arg.problem_size.m(), arg.problem_size.n());
      bool test1 =
          test(rowC, colC + 1, arg.problem_size.m(), arg.problem_size.n());

      if (test0 && test1) {
        *reinterpret_cast<float2 *>(&C_fragemnt[i * 4 + j * 2]) =
            *reinterpret_cast<float2 *>(
                &arg.C[rowC * arg.problem_size.n() + colC]);
      } else {
        C_fragemnt[i * 4 + j * 2] =
            test0 ? arg.C[rowC * arg.problem_size.n() + colC]
                  : ElementOutput(0);
        C_fragemnt[i * 4 + j * 2 + 1] =
            test1 ? arg.C[rowC * arg.problem_size.n() + colC + 1]
                  : ElementOutput(0);
      }
    }
  }
...
__device__ void loadtileA(MMAarguments &arg, ElementInputA *A, int idx) {
  ...
  for (int i = 0; i < 2; i++) {
    ...
    if (test0 && test1 && test2 && test3) {
      ...
      asm volatile("cp.async.cg.shared.global [%0], [%1], %2;\n" ::"r"(
                       (uint32_t)__cvta_generic_to_shared(&A[tileIdx])),
...
__device__ void loadtileB(MMAarguments &arg, ElementInputB *B, int idx) {
  ...
  for (int i = 0; i < 1; i++) {
    ...
    if (test0 && test1 && test2 && test3) {
      ...
      asm volatile("cp.async.cg.shared.global [%0], [%1], %2;\n" ::"r"(
                       (uint32_t)__cvta_generic_to_shared(&B[tileIdx])),
    ...
    for (int j = 0; j < 2; j++) {
      ...
      if (test0 && test1) {
        *reinterpret_cast<float2 *>(
            ...
            *reinterpret_cast<float2 *>(&C_fragment[i * 4 + j * 2]);
      } else {
        if (test0)
          arg.D[rowC * arg.problem_size.n() + colC] = C_fragment[i * 4 + j * 2];
        if (test1)
          arg.D[rowC * arg.problem_size.n() + colC + 1] =
              C_fragment[i * 4 + j * 2 + 1];
      }
    }
  }
}
...
__device__ void mma_tile_tune(MMAarguments &arg, ElementInputA *A,
                              ElementInputB *B, ElementOutput *C_fragment) {
  const int warpidx = threadIdx.x / 32;
  const int rowwarp = warpidx / 2;
  const int colwarp = warpidx % 2;
  const int laneidx = threadIdx.x % 32;
