...
__device__ void mma_tile_tune(MMAarguments &arg, ElementInputA *A,
                              ElementInputB *B, ElementInputA *A_fragment,
  ...
  /*
      Each Tile of C/D calculated by per warp is divided into four big
     Tiles(2x2), Before calculating each big tile, corresponding tile of A and B
     will be transfered from shared memory to registers. We will iterate each
     big Tile in the following order: [x][ ]   =>  [ ][x]  =>  [ ][ ]  =>  [ ][
     ] [ ][ ]   =>  [ ][ ]  =>  [ ][x]  =>  [x][ ] Thus we can reuse one tile of
     A/B instead of loading tile of A and B each time.
  */
    ...
    for (int tileidx = 0; tileidx < 4; tileidx++) {
      ...
      asm volatile("mma.sync.aligned.m16n8k8.row.col.f32.tf32.tf32.f32 "
                   ...
                   : "r"(*reinterpret_cast<uint32_t const *>(
                         &A_fragment[rowtile * 4 + 0])), // A[0]
                     "r"(*reinterpret_cast<uint32_t const *>(
                         &A_fragment[rowtile * 4 + 1])), // A[1]
                     "r"(*reinterpret_cast<uint32_t const *>(
                         &A_fragment[rowtile * 4 + 2])), // A[2]
                     "r"(*reinterpret_cast<uint32_t const *>(
                         &A_fragment[rowtile * 4 + 3])), // A[3]
                     "r"(*reinterpret_cast<uint32_t const *>(
                         &B_fragment[coltile * 2 + 0])), // B[0]
                     "r"(*reinterpret_cast<uint32_t const *>(
                         &B_fragment[coltile * 2 + 1])), // B[1]
...
__global__ void GEMM_MMA_tune(MMAarguments arg) {
  ...
  for (int i = 0; i < iters; i++) {
    if (i + 1 < iters) {
      asm("cp.async.wait_all;\n" ::);
      __syncthreads();
      mma_tile_tune(arg, tileA[i % 2], tileB[i % 2], A_fragment, B_fragment,
                    C_fragment);
      loadtileA(arg, tileA[1 - i % 2], i + 1);
      loadtileB(arg, tileB[1 - i % 2], i + 1);
    } else {
