...
struct Options {
  ...
  Options()
      : help(false), problem_size({5120, 4096, 4096}), iterations(20),
        ifprint(false) {}
...
using ElementInputA =
    cutlass::tfloat32_t; // <- data type of elements in input matrix A
using ElementInputB =
    cutlass::tfloat32_t;     // <- data type of elements in input matrix B
...
struct Index {
  ...
  __device__ Index() {
    ...
    blockIdx_y =
        (blockIdx.y << logtile) + ((blockIdx.x) & ((1 << (logtile)) - 1));

    for (int i = 0; i < 4; i++) {
      tileidx[i] = laneidx / 2 * 8 + (((laneidx >> 3) ^ laneidx) & 1) * 4 +
                   warpidx * 128 + i * 128 * 4;
    }
    ...
    for (int k = 0; k < 2; k++) {
      for (int i = 0; i < 4; i++) {
        a[k][i] = (rowwarp * 64 + i * 16 + laneidx % 16) * 8 +
                  (((laneidx >> 2) ^ (laneidx >> 4)) & 1) * 4 + k * 128 * 8;
        b[k][i] = (colwarp * 64 + i * 16 + laneidx % 8 + laneidx / 16 * 8) * 8 +
                  (((laneidx >> 2) ^ (laneidx >> 3)) & 1) * 4 + k * 128 * 8;
      }
    }
  }
};
...
__device__ int valid_num(int dim1, int dim0, int ndim1, int ndim0,
                         int max_num) {
  return dim1 < ndim1 ? std::max(0, std::min(ndim0 - dim0, max_num)) : 0;
}

__device__ void loadtileC(MMAarguments &arg, ElementOutput *C_fragemnt1,
                          ElementOutput *C_fragemnt2, Index &index) {
  int rowtile, coltile, rowC, colC, nbytes;
  for (int i = 0; i < 16; i++) {
    rowtile = i / 8;
    coltile = i % 8;
    for (int j = 0; j < 2; j++) {
      rowC = index.blockIdx_y * 128 + index.rowwarp * 64 + rowtile * 16 +
             index.laneidx / 4 + j * 8;
      colC = index.blockIdx_x * 128 + index.colwarp * 64 + coltile * 8 +
             index.laneidx % 4 * 2;

      nbytes =
          valid_num(rowC, colC, arg.problem_size.m(), arg.problem_size.n(), 2);

      if (nbytes == 2) {
        *reinterpret_cast<float2 *>(&C_fragemnt1[i * 4 + j * 2]) =
            *reinterpret_cast<float2 *>(
                &arg.C[rowC * arg.problem_size.n() + colC]);
      } else {
        C_fragemnt1[i * 4 + j * 2] =
            nbytes ? arg.C[rowC * arg.problem_size.n() + colC]
                   : ElementOutput(0);
      }
    }
  }
  ...
  for (int i = 0; i < 16; i++) {
    ...
    for (int j = 0; j < 2; j++) {
      ...
      if (nbytes == 2) {
        *reinterpret_cast<float2 *>(&C_fragemnt2[i * 4 + j * 2]) =
            *reinterpret_cast<float2 *>(
                &arg.C[rowC * arg.problem_size.n() + colC]);
      } else {
        C_fragemnt2[i * 4 + j * 2] =
...
__device__ void storetile(MMAarguments &arg, ElementOutput *C_fragment1,
                          ElementOutput *C_fragment2, Index &index) {
  int rowtile, coltile, rowC, colC, nbytes;
  ...
  for (int i = 0; i < 16; i++) {
    ...
    for (int j = 0; j < 2; j++) {
      ...
      if (nbytes == 2) {
        *reinterpret_cast<float2 *>(
            &arg.D[rowC * arg.problem_size.n() + colC]) =
            *reinterpret_cast<float2 *>(&C_fragment1[i * 4 + j * 2]);
      } else {
        if (nbytes)
          arg.D[rowC * arg.problem_size.n() + colC] =
              C_fragment1[i * 4 + j * 2];
  ...
  for (int i = 0; i < 16; i++) {
    ...
    for (int j = 0; j < 2; j++) {
      ...
      if (nbytes == 2) {
        *reinterpret_cast<float2 *>(
            &arg.D[rowC * arg.problem_size.n() + colC]) =
            *reinterpret_cast<float2 *>(&C_fragment2[i * 4 + j * 2]);
      } else {
        if (nbytes)
          arg.D[rowC * arg.problem_size.n() + colC] =
              C_fragment2[i * 4 + j * 2];
...
__device__ void mma_tile(MMAarguments &arg, ElementInputA *A_fragment,
                         ElementInputB *B_fragment, ElementOutput *C_fragment1,
                         ElementOutput *C_fragment2) {
  ...
  for (int k = 0; k < 2; k++)
    for (int i = 0; i < 16; i++) {
      asm volatile("mma.sync.aligned.m16n8k8.row.col.f32.tf32.tf32.f32 "
                   ...
                   : "r"(*reinterpret_cast<uint32_t const *>(
                         &A_fragment[i / 8 * 4 + 0 + k * 16])), // A[0]
                     "r"(*reinterpret_cast<uint32_t const *>(
                         &A_fragment[i / 8 * 4 + 1 + k * 16])), // A[1]
                     "r"(*reinterpret_cast<uint32_t const *>(
                         &A_fragment[i / 8 * 4 + 2 + k * 16])), // A[2]
                     "r"(*reinterpret_cast<uint32_t const *>(
                         &A_fragment[i / 8 * 4 + 3 + k * 16])), // A[3]
                     "r"(*reinterpret_cast<uint32_t const *>(
                         &B_fragment[i % 8 * 2 + 0 + k * 16])), // B[0]
                     "r"(*reinterpret_cast<uint32_t const *>(
                         &B_fragment[i % 8 * 2 + 1 + k * 16])), // B[1]
      ...
      asm volatile("mma.sync.aligned.m16n8k8.row.col.f32.tf32.tf32.f32 "
                   ...
                   : "r"(*reinterpret_cast<uint32_t const *>(
                         &A_fragment[i / 8 * 4 + 0 + 8 + k * 16])), // A[0]
                     "r"(*reinterpret_cast<uint32_t const *>(
                         &A_fragment[i / 8 * 4 + 1 + 8 + k * 16])), // A[1]
                     "r"(*reinterpret_cast<uint32_t const *>(
                         &A_fragment[i / 8 * 4 + 2 + 8 + k * 16])), // A[2]
                     "r"(*reinterpret_cast<uint32_t const *>(
                         &A_fragment[i / 8 * 4 + 3 + 8 + k * 16])), // A[3]
                     "r"(*reinterpret_cast<uint32_t const *>(
                         &B_fragment[i % 8 * 2 + 0 + k * 16])), // B[0]
                     "r"(*reinterpret_cast<uint32_t const *>(
                         &B_fragment[i % 8 * 2 + 1 + k * 16])), // B[1]
...
#define LDGSTSCG(smem_addr, glb_addr, nbytes, src_in_bytes)                    \
  asm volatile(                                                                \
      "cp.async.cg.shared.global.L2::128B [%0], [%1], %2, %3;\n" ::"r"(        \
          (uint32_t)__cvta_generic_to_shared(smem_addr)),                      \
      "l"(glb_addr), "n"(nbytes), "r"(src_in_bytes))
...
__device__ void loadtileA(MMAarguments &arg, ElementInputA *A, Index &index) {
  ...
  for (int k = 0; k < 2; k++) {
    for (int i = 0; i < 2; i++) {
      src_in_bytes = valid_num(index.rowA[k][i], index.colA[k][i],
                               arg.problem_size.m(), arg.problem_size.k(), 4);

      LDGSTSCG(
          &A[index.tileidx[k * 2 + i]],
          &arg.A[index.rowA[k][i] * arg.problem_size.k() + index.colA[k][i]],
          16, src_in_bytes * 4);
    }
  }
...
__device__ void loadtileB(MMAarguments &arg, ElementInputB *B, Index &index) {
  ...
  for (int k = 0; k < 2; k++) {
    for (int i = 0; i < 2; i++) {
      src_in_bytes = valid_num(index.colB[k][i], index.rowB[k][i],
                               arg.problem_size.n(), arg.problem_size.k(), 4);

      LDGSTSCG(
          &B[index.tileidx[k * 2 + i]],
          &arg.B[index.colB[k][i] * arg.problem_size.k() + index.rowB[k][i]],
          16, src_in_bytes * 4);
    }
  }
...
__device__ void epilogue(MMAarguments &arg, ElementOutput *C1,
                         ElementOutput *C2, ElementOutput *Cs, Index &index) {
  ...
  for (int i = 0; i < 32; i++) {
    ...
    MOV2(&Cs[(rowCs + 8) * 8 + colCs],
         i < 16 ? &C1[i * 4 + 2] : &C2[(i - 16) * 4 + 2]);
  }
  ...
  for (int i = 0; i < 32; i++) {
    ...
    rowC = index.blockIdx_y * 128 + index.rowwarp * 64 + i / 8 * 16 +
           index.laneidx / 2;
    colC = index.blockIdx_x * 128 + index.colwarp * 64 + i % 8 * 8 +
           index.laneidx % 2 * 4;

    nbytes =
        valid_num(rowC, colC, arg.problem_size.m(), arg.problem_size.n(), 4);
...
__global__ void GEMM_MMA_with_epilogue(MMAarguments arg) {
  ...
  epilogue(arg, C_fragment1, C_fragment2,
           reinterpret_cast<ElementOutput *>(tileA), index);
}
...
void launch_GEMM_MMA(MMAarguments &arg) {
  ...
  if (arg.alpha == 1 && arg.beta == 1) {
    CUDA_CHECK(cudaFuncSetAttribute((void *)GEMM_MMA_fast,
                                    cudaFuncAttributeMaxDynamicSharedMemorySize,
                                    smem_size));
    GEMM_MMA_fast<<<grid, block, smem_size>>>(arg);
  } else {
    CUDA_CHECK(cudaFuncSetAttribute((void *)GEMM_MMA_with_epilogue,
                                    cudaFuncAttributeMaxDynamicSharedMemorySize,
                                    smem_size));
...
int main(int argc, const char **argv) {
  ...
  cutlass::reference::device::TensorFillRandomUniform(
      tensor_c.device_view(), 3, ElementAccumulator(4.f),
      ElementAccumulator(-4.f), 0);
  ...
  timer.bind_run(
      ...
      [&] {
        ...
        typename Gemm::Arguments arguments{
            options.problem_size,  // <- problem size of matrix multiplication
  ...
  //////////////////////GEMM_MMA///////////////////////
  {
    ...
    timer.bind_run(
        "MMA_tune",
        [&] {
          MMAarguments mmaArg{options.problem_size,
    ...
    timer.testEqual<ElementOutput, LayoutOutput>("MMA_tune==ref", tensor_d,
                                                 tensor_mma_d, options.ifprint);
