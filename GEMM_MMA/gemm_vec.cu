...
__device__ void loadtileC(MMAarguments &arg, ElementOutput *C) {
  ...
  for (int i = 0; i < 64; i++) {
    ...
    C[tileIdx] = rowC < arg.problem_size.m() && colC < arg.problem_size.n()
...
__device__ void loadtileA(MMAarguments &arg, ElementInputA *A, int idx) {
  ...
  for (int i = 0; i < 8; i++) {
    ...
    A[tileIdx] = rowA < arg.problem_size.m() && colA < arg.problem_size.k()
                     ? arg.A[rowA * arg.problem_size.k() + colA]
                     : ElementInputA(0);
  }
}
...
__device__ void loadtileB(MMAarguments &arg, ElementInputB *B, int idx) {
  ...
  for (int i = 0; i < 4; i++) {
    ...
    B[tileIdx] = rowB < arg.problem_size.k() && colB < arg.problem_size.n()
                     ? arg.B[colB * arg.problem_size.k() + rowB]
                     : ElementInputB(0);
  }
}
...
int main(int argc, char **argv) {
...

  {
    ...
    timer.testEqual<ElementOutput, LayoutOutput>("MMA_vec==ref", tensor_d,
                                                 tensor_mma_d);
  }
}