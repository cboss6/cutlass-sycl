...
int main(int argc, char** argv)
{
  ...
  for (int j = 0; j < m*n; ++j) h_C_ref[j] = static_cast<TC>(-1);
  ...
  TB* d_B;
  TC* d_C;
  cudaMalloc(&d_A, m*k*sizeof(TA));
  cudaMalloc(&d_B, k*n*sizeof(TB));
  cudaMalloc(&d_C, m*n*sizeof(TC));

  cudaMemcpy(d_A, h_A.data(), m*k*sizeof(TA), cudaMemcpyHostToDevice);
  cudaMemcpy(d_B, h_B.data(), k*n*sizeof(TB), cudaMemcpyHostToDevice);
  cudaMemcpy(d_C, h_C.data(), m*n*sizeof(TC), cudaMemcpyHostToDevice);
  ...
  gemm(transA, transB, m, n, k,
       ...
       beta,
       d_C, ldC);
  cudaMemcpy(h_C.data(), d_C, m*n*sizeof(TC), cudaMemcpyDeviceToHost);
  cudaDeviceSynchronize();
  ...
  bool failed = false;
  for (int j = 0; j < m*n; ++j) {
    if (fabs(h_C[j] - h_C_ref[j]) > 1e-5) {
      DBG("Failed: C[%d] = %f, C_ref[%d] = %f\n", j, h_C[j], j, h_C_ref[j]);
      failed = true;
      break;
    }
  }
