...
__device__ void loadtileC(MMAarguments &arg, ElementOutput *C) {
  ...
  for (int i = 0; i < 16; i++) {
    ...
    bool test0 =
        test(rowC_0, colC_0, arg.problem_size.m(), arg.problem_size.n());
    bool test1 =
        test(rowC_1, colC_1, arg.problem_size.m(), arg.problem_size.n());
    bool test2 =
        test(rowC_2, colC_2, arg.problem_size.m(), arg.problem_size.n());
    bool test3 =
        test(rowC_3, colC_3, arg.problem_size.m(), arg.problem_size.n());

    if (test0 && test1 && test2 && test3) {
      *reinterpret_cast<float4 *>(&C[tileIdx]) = *reinterpret_cast<float4 *>(
          &arg.C[rowC_0 * arg.problem_size.n() + colC_0]);
    } else {
      C[tileIdx] = test0 ? arg.C[rowC_0 * arg.problem_size.n() + colC_0]
                         : ElementOutput(0);
      C[tileIdx + 1] = test1 ? arg.C[rowC_1 * arg.problem_size.n() + colC_1]
                             : ElementOutput(0);
      C[tileIdx + 2] = test2 ? arg.C[rowC_2 * arg.problem_size.n() + colC_2]
                             : ElementOutput(0);
      C[tileIdx + 3] = test3 ? arg.C[rowC_3 * arg.problem_size.n() + colC_3]
                             : ElementOutput(0);
    }
  }
}
...
__device__ void loadtileA(MMAarguments &arg, ElementInputA *A, int idx) {
  ...
  for (int i = 0; i < 2; i++) {
    ...
    bool test0 =
        test(rowA_0, colA_0, arg.problem_size.m(), arg.problem_size.k());
    bool test1 =
        test(rowA_1, colA_1, arg.problem_size.m(), arg.problem_size.k());
    bool test2 =
        test(rowA_2, colA_2, arg.problem_size.m(), arg.problem_size.k());
    bool test3 =
        test(rowA_3, colA_3, arg.problem_size.m(), arg.problem_size.k());

    if (test0 && test1 && test2 && test3) {
      *reinterpret_cast<float4 *>(&A[tileIdx]) = *reinterpret_cast<float4 *>(
          &arg.A[rowA_0 * arg.problem_size.k() + colA_0]);
    } else {
      A[tileIdx] = test0 ? arg.A[rowA_0 * arg.problem_size.k() + colA_0]
                         : ElementInputA(0);
      A[tileIdx + 1] = test1 ? arg.A[rowA_1 * arg.problem_size.k() + colA_1]
                             : ElementInputA(0);
      A[tileIdx + 2] = test2 ? arg.A[rowA_2 * arg.problem_size.k() + colA_2]
                             : ElementInputA(0);
      A[tileIdx + 3] = test3 ? arg.A[rowA_3 * arg.problem_size.k() + colA_3]
                             : ElementInputA(0);
    }
  }
}
...
__device__ void loadtileB(MMAarguments &arg, ElementInputB *B, int idx) {
  ...
  for (int i = 0; i < 1; i++) {
    ...
    bool test0 =
        test(rowB_0, colB_0, arg.problem_size.k(), arg.problem_size.n());
    bool test1 =
        test(rowB_1, colB_1, arg.problem_size.k(), arg.problem_size.n());
    bool test2 =
        test(rowB_2, colB_2, arg.problem_size.k(), arg.problem_size.n());
    bool test3 =
        test(rowB_3, colB_3, arg.problem_size.k(), arg.problem_size.n());

    if (test0 && test1 && test2 && test3) {
      *reinterpret_cast<float4 *>(&B[tileIdx]) = *reinterpret_cast<float4 *>(
          &arg.B[colB_0 * arg.problem_size.k() + rowB_0]);
    } else {
      B[tileIdx] = test0 ? arg.B[colB_0 * arg.problem_size.k() + rowB_0]
                         : ElementInputB(0);
      B[tileIdx + 1] = test1 ? arg.B[colB_1 * arg.problem_size.k() + rowB_1]
                             : ElementInputB(0);
      B[tileIdx + 2] = test2 ? arg.B[colB_2 * arg.problem_size.k() + rowB_2]
                             : ElementInputB(0);
      B[tileIdx + 3] = test3 ? arg.B[colB_3 * arg.problem_size.k() + rowB_3]
                             : ElementInputB(0);
    }
  }
}
...
__device__ void storetile(MMAarguments &arg, ElementOutput *D) {
  ...
  for (int i = 0; i < 16; i++) {
    ...
    bool test0 =
        test(rowD_0, colD_0, arg.problem_size.m(), arg.problem_size.n());
    bool test1 =
        test(rowD_1, colD_1, arg.problem_size.m(), arg.problem_size.n());
    bool test2 =
        test(rowD_2, colD_2, arg.problem_size.m(), arg.problem_size.n());
    bool test3 =
        test(rowD_3, colD_3, arg.problem_size.m(), arg.problem_size.n());

    if (test0 && test1 && test2 && test3)
      *reinterpret_cast<float4 *>(
          &arg.D[rowD_0 * arg.problem_size.n() + colD_0]) =
          *reinterpret_cast<float4 *>(&D[tileIdx]);
    else {
      if (test0)
...
__device__ void mma_tile(MMAarguments &arg, ElementInputA *A, ElementInputB *B,
                         ElementAccumulator *C, ElementOutput *D) {
  ...
  for (int tileidx = 0; tileidx < 16; tileidx++) {
    ...
    cd[0] = (rowwarp * 64 + rowtile * M + laneidx / 4) * 64 + colwarp * 32 +
            coltile * N + laneidx % 4 * 2;
    cd[1] = cd[0] + 1;
    cd[2] = cd[0] + 8 * 64;
    cd[3] = cd[2] + 1;
...
void launch_GEMM_MMA(MMAarguments &arg) {
  ...
  | C[0:127][0:63]   | C[0:127][64:127] | C[0:127][128:191]|
  | 处理128x64区域   | 处理128x64区域   | 处理128x64区域   |
  +------------------+------------------+------------------+
  | Block(1,0)       | Block(1,1)       | Block(1,2)       |
  | C[128:255][0:63] | C[128:255][64:127]| C[128:255][128:191]|
  | 处理128x64区域   | 处理128x64区域   | 处理128x64区域   |
  +------------------+------------------+------------------+
  ...
  +------------------+------------------+
  | Warp 0           | Warp 1           |
  | C[0:63][0:31]    | C[0:63][32:63]   |
  | 处理64x32区域    | 处理64x32区域    |
  +------------------+------------------+
  | Warp 2           | Warp 3           |
  | C[64:127][0:31]  | C[64:127][32:63] |
  | 处理64x32区域    | 处理64x32区域    |
  +------------------+------------------+
  ...
  | C[0:7][0:3]      | C[8:15][0:3]     |
  | 处理8x4区域      | 处理8x4区域      |
  +------------------+------------------+
  | Thread 16-23     | Thread 24-31     |
  | C[16:23][0:3]    | C[24:31][0:3]    |
  | 处理8x4区域      | 处理8x4区域      |
  +------------------+------------------+
  */
  dim3 grid, block;
...
int main(int argc, char **argv) {
  ...
  //     MMAarguments mmaArg{problem_size, tensor_a.device_data(),
  //                         tensor_b.device_data(), tensor_c.device_data(),
  //                         tensor_mma_d.device_data()};
  //     launch_GEMM_MMA_bf(mmaArg);
  //   });

  //   timer.testEqual<ElementOutput, LayoutOutput>("MMA_bf==ref", tensor_d,
  //                                                tensor_mma_d);
  // }

  {
    ...
    timer.testEqual<ElementOutput, LayoutOutput>("MMA_bf_co==ref", tensor_d,
                                                 tensor_mma_d);
  }
}
